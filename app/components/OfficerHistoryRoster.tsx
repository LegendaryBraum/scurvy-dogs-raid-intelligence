...
export function OfficerHistoryRoster({ players, activeKeys, difficultyLabel, expandedPlayerId, notes, busy, loading, status, onTogglePlayer, onSaveNote, onDeleteNote }: {
  ...
  return <article className="panel officer-history-roster">
    <div className="panel-heading"><div><p className="eyebrow muted"><span /> {difficultyLabel} roster history</p><h2>One row per raider, every matching night</h2><p>These are identity-aware {difficultyLabel.toLowerCase()} averages—not a single selected pull. Select a raider to open their boss history and individual kills.</p></div><span className="confidence">{players.length} raiders</span></div>
