...
type RuleAuditResult = {
  ...
  difficulty: string;
  pull: RuleAuditPull;
  rule: { id: string; name: string; spellId: number; category: string; severity: string; weight: number; eventType: string; scoringMode: "penalty" | "success" };
  summary: { participantCount: number; matchedPlayers: number; unmatchedPlayers: number; totalMatches: number; totalDeduction: number; previousMatches: number };
  warnings: string[];
  players: Array<{ playerId: string; name: string; realm: string; role: string; currentScore: number; projectedScore: number; previousMatches: number; matches: number; deduction: number; events: Array<{ seconds: number; amount: number | null; detail: string }> }>;
};
...
const ruleDifficulties = ["Normal", "Heroic", "Mythic"] as const;
type RuleDifficulty = typeof ruleDifficulties[number];
...
function WipefestCalibrationWizard({ disabled, onBossIdentified, onSaved, onReanalysisComplete }: { disabled: boolean; onBossIdentified: (bossId: string, difficulty: string) => void; onSaved: (rules: MechanicRule[]) => void; onReanalysisComplete: () => Promise<void> }) {
  ...
  useEffect(() => {
    let active = true;
    fetch("/api/reanalysis-jobs", { cache: "no-store" })
      .then(async (response) => response.ok ? response.json() as Promise<{ jobs: ReanalysisJob[] }> : null)
      .then((result) => {
        if (!active || !result?.jobs?.length) return;
        setReanalysisJobs(result.jobs);
        setStatus("An unfinished score recalculation was found. Continue from its saved checkpoint whenever Warcraft Logs is ready.");
      })
      .catch(() => undefined);
    return () => { active = false; };
  }, []);

  function upsertReanalysisJob(nextJob: ReanalysisJob) {
    setReanalysisJobs((current) => [nextJob, ...current.filter((job) => job.id !== nextJob.id)]);
  }

  async function loadReanalysisPlan(bossId: string, difficulty: string) {
    try {
      const response = await fetch(`/api/reanalysis-jobs?bossId=${encodeURIComponent(bossId)}&difficulty=${encodeURIComponent(difficulty)}`, { cache: "no-store" });
      const result = await response.json() as { plan?: ReanalysisPlan | null; jobs?: ReanalysisJob[] };
      if (!response.ok) return null;
      if (result.jobs) setReanalysisJobs(result.jobs);
      setReanalysisPlan(result.plan ?? null);
  ...
  function reset() {
    ...
    setReanalysisPlan(null);
    setReanalysisJobs((current) => current.filter((job) => job.status !== "completed"));
  }
  ...
  async function readWipefest(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/calibration", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ wipefestUrl }) });
      ...
      onBossIdentified(result.preview.bossId, result.preview.difficulty);
      ...
      setStatus(newRuleCount > 0
        ? `${result.preview.candidates.length} ${result.preview.difficulty} mechanics are ready for your final review. Nothing affects player scores until you save the checked rules.`
        : `${result.preview.bossName} is already configured for ${result.preview.difficulty}. Its existing rules are shown read-only here—no duplicate save is needed.`);
  ...
  async function saveSelected() {
    if (!preview) { setStatus("Read a Wipefest pull before saving rules."); return; }
    ...
    try {
      ...
      const nextPlan = await loadReanalysisPlan(preview.bossId, preview.difficulty);
      setStatus(`${results.length} rule${results.length === 1 ? "" : "s"} saved for ${preview.bossName} ${preview.difficulty}. ${nextPlan?.pullCount ? `${nextPlan.pullCount} existing pull${nextPlan.pullCount === 1 ? " is" : "s are"} ready to be recalculated.` : "Future imports will use this configuration automatically."}`);
  ...
  async function runReanalysis(existingJob?: ReanalysisJob) {
    ...
    try {
      ...
      if (currentJob.status === "completed") {
        await onReanalysisComplete();
        setReanalysisPlan(null);
        setStatus(`${currentJob.totalPulls} ${currentJob.bossName} ${currentJob.difficulty} pull${currentJob.totalPulls === 1 ? "" : "s"} recalculated with ${currentJob.rules} rules. ${currentJob.events} mechanic event${currentJob.events === 1 ? "" : "s"} were found, and every player and officer view is now current.`);
      } else {
        setStatus("The recalculation reached a safe checkpoint. Continue it to finish the remaining pulls.");
      }
    } catch (error) { setStatus(error instanceof Error ? error.message : "The stored pulls could not be recalculated."); }
    finally { setBusy(false); }
  }
  ...
  return <article className={`panel calibration-wizard ${preview ? "calibration-wizard-review" : ""}`}>
    ...
    {!preview ? <form className="calibration-form" onSubmit={readWipefest}><label>Wipefest boss pull URL<input value={wipefestUrl} onChange={(event) => setWipefestUrl(event.target.value)} placeholder="https://www.wipefest.gg/report/.../fight/last" required /></label><button className="primary-button" disabled={busy || disabled} type="submit">{busy ? "Identifying boss…" : "Read boss mechanics"}</button></form> : <>
...
function RuleAuditPanel({ bossId, bossName, difficulty, rules, changedRuleIds, disabled, onApprove }: { bossId: string; bossName: string; difficulty: RuleDifficulty; rules: MechanicRule[]; changedRuleIds: string[]; disabled: boolean; onApprove: () => Promise<void> }) {
  const testableRules = rules.filter((rule) => rule.enabled !== false && rule.condition.scoringMode !== "context");
  const changedTestableRules = testableRules.filter((rule) => changedRuleIds.includes(rule.id));
  const [pulls, setPulls] = useState<RuleAuditPull[]>([]);
  const [pullId, setPullId] = useState("");
  const [ruleId, setRuleId] = useState(changedTestableRules[0]?.id ?? testableRules[0]?.id ?? "");
  const [audits, setAudits] = useState<Record<string, RuleAuditResult>>({});
  const [busy, setBusy] = useState(false);
  const [status, setStatus] = useState("Loading saved pulls…");

  useEffect(() => {
    let active = true;
    fetch(`/api/rule-audit?bossId=${encodeURIComponent(bossId)}&difficulty=${encodeURIComponent(difficulty)}`, { cache: "no-store" })
      .then(async (response) => {
        const result = await response.json() as { pulls?: RuleAuditPull[]; error?: string };
        if (!response.ok) throw new Error(result.error ?? "The saved pulls could not be loaded.");
        if (!active) return;
        const nextPulls = result.pulls ?? [];
        setPulls(nextPulls);
        setPullId((current) => nextPulls.some((pull) => pull.id === current) ? current : nextPulls[0]?.id ?? "");
        setStatus(nextPulls.length ? "Choose one rule and one representative pull. Testing is read-only." : `No saved ${difficulty} pulls are available for ${bossName}.`);
      })
      .catch((error) => { if (active) setStatus(error instanceof Error ? error.message : "The saved pulls could not be loaded."); });
    return () => { active = false; };
  }, [bossId, bossName, difficulty]);

  const selectedRule = testableRules.find((rule) => rule.id === ruleId) ?? testableRules[0];
  const selectedAudit = selectedRule ? audits[selectedRule.id] : undefined;
  const testedChangedCount = changedTestableRules.filter((rule) => Boolean(audits[rule.id])).length;
  const allChangedRulesTested = changedTestableRules.every((rule) => Boolean(audits[rule.id]));
  const changedCount = changedRuleIds.length;

  async function testRule() {
    if (!selectedRule || !pullId) { setStatus("Choose one rule and one pull to test."); return; }
    setBusy(true);
    setStatus(`Reading ${selectedRule.name} on the selected pull…`);
    try {
      const response = await fetch("/api/rule-audit", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ bossId, difficulty, pullId, ruleId: selectedRule.id }) });
      const result = await response.json() as { audit?: RuleAuditResult; error?: string };
      if (!response.ok || !result.audit) throw new Error(result.error ?? "The rule test could not be completed.");
      setAudits((current) => ({ ...current, [selectedRule.id]: result.audit! }));
      setStatus(result.audit.warnings.length ? "Test complete. Review the warning before approving historical updates." : "Test complete. The match count and score effect are ready for review.");
    } catch (error) { setStatus(error instanceof Error ? error.message : "The rule test could not be completed."); }
    finally { setBusy(false); }
  }

  return <section className="panel rule-audit-panel">
    <div className="rule-audit-heading"><div><p className="eyebrow muted"><span /> Scoring audit</p><h2>Test one pull before changing history</h2><p>This reads Warcraft Logs without changing a stored score. Confirm who matched, the exact deduction, and the projected result before applying changed rules to older pulls.</p></div><span className={`rule-audit-state ${changedCount === 0 ? "current" : allChangedRulesTested ? "ready" : "review"}`}>{changedCount === 0 ? "All current" : allChangedRulesTested ? "Ready to apply" : `${testedChangedCount}/${changedTestableRules.length} tested`}</span></div>
    <div className="rule-audit-controls"><label>Rule to test<select disabled={busy || !testableRules.length} value={selectedRule?.id ?? ""} onChange={(event) => { setRuleId(event.target.value); setStatus("Choose a pull, then test this rule without changing stored scores."); }}>{testableRules.map((rule) => <option key={rule.id} value={rule.id}>{changedRuleIds.includes(rule.id) ? "Changed · " : "Spot check · "}{rule.name} · {rule.condition.scoringMode === "success" ? "evidence only" : `−${rule.weight}`}</option>)}</select></label><label>Representative pull<select disabled={busy || !pulls.length} value={pullId} onChange={(event) => { setPullId(event.target.value); setStatus("Ready for a read-only rule test."); }}>{pulls.map((pull) => <option key={pull.id} value={pull.id}>{pull.label} · {pull.result} · {pull.duration}</option>)}</select></label><button className="primary-button" disabled={busy || disabled || !selectedRule || !pullId} onClick={testRule} type="button">{busy ? "Testing against Warcraft Logs…" : selectedAudit?.pull.id === pullId ? "Retest this rule" : "Test this rule"}</button></div>
    {status && <p className="rule-audit-status" role="status">{status}</p>}
    {selectedAudit && <div className="rule-audit-results"><div className="rule-audit-summary"><span><small>Players matched</small><strong>{selectedAudit.summary.matchedPlayers}/{selectedAudit.summary.participantCount}</strong></span><span><small>New matches</small><strong>{selectedAudit.summary.totalMatches}</strong><em>{selectedAudit.summary.previousMatches} currently stored</em></span><span><small>Points removed</small><strong>{selectedAudit.rule.scoringMode === "penalty" ? `−${selectedAudit.summary.totalDeduction}` : "0"}</strong><em>Across the active roster</em></span><span><small>Tested pull</small><strong>{selectedAudit.pull.result}</strong><em>{selectedAudit.pull.label}</em></span></div>{selectedAudit.warnings.length > 0 && <div className="rule-audit-warnings">{selectedAudit.warnings.map((warning) => <p key={warning}><b>Check this:</b> {warning}</p>)}</div>}{selectedAudit.players.length > 0 ? <div className="rule-audit-player-list">{selectedAudit.players.map((player) => <article key={player.playerId}><div className="rule-audit-player"><span>{player.name.slice(0, 2).toUpperCase()}</span><div><strong>{player.name}</strong><small>{player.role}{player.realm ? ` · ${player.realm}` : ""}</small></div></div><span><small>Matches</small><strong>{player.matches}</strong><em>{player.previousMatches} stored</em></span><span><small>This rule</small><strong>{player.deduction ? `−${player.deduction}` : "Evidence"}</strong></span><span className="audit-score-change"><small>Mechanics</small><strong>{player.currentScore} <b>→</b> {player.projectedScore}</strong></span><details><summary>View matching events</summary><div>{player.events.map((event, index) => <p key={`${event.seconds}-${index}`}><time>{Math.floor(event.seconds / 60)}:{String(event.seconds % 60).padStart(2, "0")}</time><span>{event.detail}</span></p>)}</div></details></article>)}</div> : <div className="rule-audit-empty"><strong>No player matches found</strong><span>The stored scores are unchanged. Check the Spell ID and event type, or try a pull where the mechanic occurred.</span></div>}<div className="rule-audit-formula"><span><strong>100</strong><small>Starting Mechanics</small></span><b>−</b><span><strong>other rule penalties</strong><small>Already stored on this pull</small></span><b>−</b><span><strong>{selectedAudit.rule.scoringMode === "penalty" ? `${selectedAudit.rule.weight} × matches` : "0 evidence points"}</strong><small>{selectedAudit.rule.name}</small></span><b>=</b><span><strong>projected score</strong><small>Shown for each matched player</small></span></div></div>}
    <div className="rule-audit-approval"><div><strong>{changedCount === 0 ? "No historical update is waiting" : allChangedRulesTested ? "Review complete" : `${changedTestableRules.length - testedChangedCount} changed scoring rule${changedTestableRules.length - testedChangedCount === 1 ? "" : "s"} still need a test`}</strong><span>{changedCount === 0 ? "You can still spot-check any rule without changing data." : changedTestableRules.length === 0 ? "Only paused-rule cleanup is waiting, so no live event test is required." : "Each changed scoring rule must be tested once. Paused rules are cleanup-only and do not require a live test."}</span></div><button disabled={disabled || busy || changedCount === 0 || !allChangedRulesTested} onClick={onApprove} type="button">Approve & update {changedCount} changed rule{changedCount === 1 ? "" : "s"}</button></div>
  </section>;
}
...
export function RaidApp({ initialData: fallbackData }: { initialData: DashboardData }) {
  ...
  const [ruleDifficulty, setRuleDifficulty] = useState<RuleDifficulty>("Normal");
  ...
  const ruleBossDifficultyRules = ruleBossRules.filter((rule) => rule.difficulties.length === 0 || rule.difficulties.includes(ruleDifficulty));
  const activeRuleBossRules = ruleBossDifficultyRules.filter((rule) => rule.enabled !== false);
  const ruleBossPulls = initialData.pulls.filter((candidate) => candidate.bossId === ruleBossId && candidate.difficulty === ruleDifficulty);
  const configuredBossCount = configBosses.filter((candidate) => activeRules.some((rule) => rule.bossId === candidate.id)).length;
  const rosterMembers: RosterMember[] = initialData.roster ?? initialData.players.map((candidate) => ({ id: candidate.id, name: candidate.name, realm: candidate.realm, className: candidate.className, spec: candidate.spec, role: candidate.role, pullsSeen: initialData.pulls.length, raidNights: 1, lastSeen: null, included: true }));
  ...
  async function addRule(event: FormEvent<HTMLFormElement>) {
    ...
    const nextRule: MechanicRule = {
      id: editingRule?.id ?? `rule-${crypto.randomUUID()}`, bossId: ruleBossId, spellId: Number(form.get("spellId")), name: String(form.get("name") ?? ""),
    ...
    <main className="shell">
      ...
      {view === "configure" && <section className="dashboard config-view" id="configure">
        ...
        {configureSection === "scoring" && <div aria-labelledby="configure-scoring-tab" className="configure-section" id="configure-scoring" role="tabpanel">
          ...
          <WipefestCalibrationWizard disabled={busy} onBossIdentified={(identifiedBossId, identifiedDifficulty) => { if (configBosses.some((candidate) => candidate.id === identifiedBossId)) { setRuleBossId(identifiedBossId); if (ruleDifficulties.includes(identifiedDifficulty as RuleDifficulty)) setRuleDifficulty(identifiedDifficulty as RuleDifficulty); setEditingRule(null); } }} onSaved={(savedRules) => { const savedBossId = savedRules[0]?.bossId; const savedBoss = configBosses.find((candidate) => candidate.id === savedBossId); const savedDifficulty = savedRules[0]?.difficulties[0]; if (savedBossId) setRuleBossId(savedBossId); if (ruleDifficulties.includes(savedDifficulty as RuleDifficulty)) setRuleDifficulty(savedDifficulty as RuleDifficulty); setRules((current) => [...savedRules, ...current]); setRuleStatus(`${savedRules.length} Wipefest-reviewed ${savedDifficulty ?? "difficulty-specific"} rule${savedRules.length === 1 ? "" : "s"} saved for ${savedBoss?.name ?? "the detected boss"}.`); }} onReanalysisComplete={refreshAnalysisViews} />
