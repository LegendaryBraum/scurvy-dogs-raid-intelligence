      ...
      {view === "configure" && <section className="dashboard config-view" id="configure">
        ...
        {configureSection === "scoring" && <div aria-labelledby="configure-scoring-tab" className="configure-section" id="configure-scoring" role="tabpanel">
          ...
          <section className="panel boss-rule-board">
            <div className="boss-rule-board-heading"><div><p className="eyebrow muted"><span /> {initialData.raid} configuration</p><h2>Choose a boss, then build its rules</h2><p>Every Venomous Abyss boss has a separate workspace. Rule counts and calibration status stay visible here so Ula&apos;Tek never looks like the only configured encounter.</p></div><div className="boss-rule-progress"><strong>{configuredBossCount}/{initialData.bosses.length}</strong><small>Bosses with active rules</small></div></div>
