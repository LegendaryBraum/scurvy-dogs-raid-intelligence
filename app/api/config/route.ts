...
export async function POST(request: Request) {
  try {
    ...
    await db.prepare("INSERT INTO mechanic_rules (id, boss_id, spell_id, name, icon, category, severity, weight, event_type, difficulties_json, roles_json, condition_json, enabled, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP) ON CONFLICT(id) DO UPDATE SET spell_id = excluded.spell_id, name = excluded.name, icon = excluded.icon, category = excluded.category, severity = excluded.severity, weight = excluded.weight, event_type = excluded.event_type, difficulties_json = excluded.difficulties_json, roles_json = excluded.roles_json, condition_json = excluded.condition_json, enabled = excluded.enabled, updated_at = CURRENT_TIMESTAMP")
      .bind(rule.id, rule.bossId, Number(rule.spellId), rule.name.trim(), rule.icon ?? null, rule.category, rule.severity, Number(rule.weight), rule.eventType, JSON.stringify(rule.difficulties), JSON.stringify(rule.roles), JSON.stringify(rule.condition), rule.enabled === false ? 0 : 1).run();
