...
import type { PlayerHistoryPoint } from "./types";
...
export async function loadPlayerHistory(playerId: string, selectedSeasonId?: string | null) {
  ...
  const season = selectedSeasonId ? { season_id: selectedSeasonId } : await db.prepare(`
    SELECT rn.season_id
    FROM raid_nights rn JOIN reports r ON r.raid_night_id = rn.id
    WHERE r.source_mode = 'live' AND rn.included = 1 AND r.included = 1
  ...
  const [historyResult, characterResult] = await Promise.all([
    ...
    `).bind(identity.identity_id, identity.id).all<{ name: string }>(),
  ]);
  ...
  return { history, linkedCharacters: characterResult.results.map((row) => row.name), identityId: identity.identity_id };
}
