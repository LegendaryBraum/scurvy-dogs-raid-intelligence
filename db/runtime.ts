...
export async function ensureSchema() {
  ...
  const statements = [
    ...
    `CREATE TABLE IF NOT EXISTS import_jobs (id TEXT PRIMARY KEY, report_code TEXT NOT NULL, report_url TEXT NOT NULL, season_id TEXT NOT NULL REFERENCES seasons(id), raid_night_id TEXT NOT NULL REFERENCES raid_nights(id), replace_existing INTEGER NOT NULL DEFAULT 0, selected_fight_ids_json TEXT NOT NULL DEFAULT '[]', completed_fight_ids_json TEXT NOT NULL DEFAULT '[]', snapshot_json TEXT, staging_report_id TEXT, status TEXT NOT NULL DEFAULT 'queued', total_pulls INTEGER NOT NULL DEFAULT 0, completed_pulls INTEGER NOT NULL DEFAULT 0, current_label TEXT, player_rows INTEGER NOT NULL DEFAULT 0, event_rows INTEGER NOT NULL DEFAULT 0, error_message TEXT, retry_after_seconds INTEGER, resume_after TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, completed_at TEXT)`,
    ...
    `CREATE TABLE IF NOT EXISTS officer_notes (id TEXT PRIMARY KEY, player_id TEXT NOT NULL REFERENCES players(id), author_officer_id TEXT NOT NULL REFERENCES officers(id), raid_night_id TEXT REFERENCES raid_nights(id), boss_id TEXT REFERENCES bosses(id), pull_id TEXT REFERENCES pulls(id), visibility TEXT NOT NULL DEFAULT 'player', body TEXT NOT NULL, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`,
    ...
    `CREATE INDEX IF NOT EXISTS idx_import_jobs_report_code ON import_jobs(report_code)`,
    `CREATE INDEX IF NOT EXISTS idx_pulls_report_included ON pulls(report_id, included)`,
    ...
    `CREATE INDEX IF NOT EXISTS idx_officer_notes_player_visibility ON officer_notes(player_id, visibility, created_at)`,
    `CREATE INDEX IF NOT EXISTS idx_officer_notes_pull ON officer_notes(pull_id)`,
